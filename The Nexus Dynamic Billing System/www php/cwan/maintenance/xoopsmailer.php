<?php
// $Id: xoopsmailer.php,v 1.17 2004/06/14 14:22:11 skalpa Exp $
//  ------------------------------------------------------------------------ //
 ...
 */
include_once("mail/xoopsmultimailer.php");
...
class XoopsMailer
{
	...
	function send($debug = false)
	{
		...
		if ( $this->body == "" && $this->template == "" ) {
			...
			$path = ( $this->templatedir != "" ) ? $this->templatedir."".$this->template : ("/language/".$xoopsConfig['language']."/mail_template/".$this->template);
			if ( !($fd = @fopen($path, 'r')) ) {
				if ($debug) {
