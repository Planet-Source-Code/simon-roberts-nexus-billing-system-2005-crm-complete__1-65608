...
'
    ...
    Case 0
        bResult = MySQL.OpenTable(ADOConn, rsSave, , "select * from radiuspools lIMIT 1")
        rsSave.AddNew
    Case Else
        bResult = MySQL.OpenTable(ADOConn, rsSave, , "select * from radiuspools where RecID = " & txtPoolDesc.Tag)
    End Select
    ...
    Select Case txtPoolDesc.Tag
    Case 0
        txtPoolDesc.Tag = MySQL.SetRecID(rsSave, "radiuspools", ADOConn)
    ...
    MySQL.Execute ADOConn, "UPDATE radiuspools SET ufPassword=AES_ENCRYPT('" + txtRadiusField(7).Text + "','" & odb.colSalts.ReturnSalt("md5Password") & "'), rlPassword=AES_ENCRYPT('" + txtRadiusField(4).Text + "','" & odb.colSalts.ReturnSalt("md5Password") & "') Where RecID = " & txtPoolDesc.Tag
...
'
    ...
    bResult = MySQL.OpenTable(ADOConn, rsLoad, , "select Description, rlFTPServer, rlTagetDir, rlFilename, rlUsername, AES_DECRYPT(rlPassword,'" & odb.colSalts.ReturnSalt("md5Password") & "') as rlPassword, rlPort, ufFTPServer, ufTagetDir, ufFilename, ufUsername, AES_DECRYPT(ufPassword,'" & odb.colSalts.ReturnSalt("md5Password") & "') as ufPassword, ufPort from radiuspools where RecID = " & Mid(Item.Key, 2))
