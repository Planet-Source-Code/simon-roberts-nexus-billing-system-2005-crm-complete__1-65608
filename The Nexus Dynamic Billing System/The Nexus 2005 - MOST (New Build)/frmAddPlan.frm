...
'
    ...
    Call MySQL.OpenTable(ADOConn, rsload, , "select plantemplates.ProductText as ProductText from plantemplates, plantypes where plantypes.TemplateID = plantemplates.RecID and plantypes.RecID = " & Mid(Item.Key, 2))
...
'
    ...
    While Not rsEx.EOF And Err.Number = 0
        ...
        br = MySQL.OpenTable(ADOConn, rsPt, , "select PeriodFee, FeePerBlock, ExtraPerHour from plantemplates where RecID = " & rsEx!TemplateID & " Limit 1")
        oItem.cJoiningFee = 0
        oItem.cPerHour = 0
        oItem.cPerMBBlock = 0
        oItem.cPeriodFee = 0
...
'
    ...
    cmbAllPLans.Clear
    bResult = MySQL.OpenTable(ADOConn, rsPlanType, , "select * from plantypes")
    ...
    If Login.lAgencyID <> 0 Then
        bResult = MySQL.OpenTable(ADOConn, rsPlanType, , "select plantypes.* from plantypes, agencyplans Where agencyplans.ptRecID = plantypes.RecID and agencyplans.IsAvailable = -1 and ServiceID = " & Mid(Node.Key, 2))
    Else
        bResult = MySQL.OpenTable(ADOConn, rsPlanType, , "select * from plantypes Where ServiceID = " & Mid(Node.Key, 2) & " and VirtualID = " & Login.lVirtualID)
    End If
    
    Call MySQL.fillLV(ADOConn, rsPlanType, lvAccounts, False)
...
'
    ...
    If lServiceID <> 0 Then
        bResult = MySQL.OpenTable(ADOConn, rsload, , "select * from servicetypes where RecID = " & lServiceID)
        If Val(rsload!ListOnRadius) = -1 Then
            bResult = MySQL.OpenTable(ADOConn, rsload, , "select * from servicetypes where ListOnRadius = -1")
        End If
        
        lblTitle = "Change Service/Plan"
    Else
...
'
    ...
    br = MySQL.OpenTable(ADOConn, rsSv, , "select * from servicetypes")
    br = MySQL.OpenTable(ADOConn, rsPt, , "select * from plantypes where RecID = " & ptRecID & " Limit 1")
    br = MySQL.OpenTable(ADOConn, rsEx, , "select flags_plantype.*, plantypes.SessionsAllowed, plantypes.SessionTimeout, plantypes.IdleTimeout, plantypes.TemplateID, plantypes.ServiceID, plantypes.MBQuota, plantypes.CostPrice, plantypes.JoiningFee, plantypes.BillImmediately, plantypes.chgInterval, plantypes.chgIntervalType, plantypes.ExtraPerHour, plantypes.FeePerBlock, plantypes.PeriodFee, plantypes.Description, plantypes.RecID as ExtraRecID, plantypes.PeriodFee, plantypes.MBPerPeriod from flags_plantype, plantypes where PlanType = plantypes.RecID AND flags_plantype.Checked = -1 and ptRecID = " & ptRecID)
    ...
    br = MySQL.OpenTable(ADOConn, rsPt, , "select PeriodFee, FeePerBlock, ExtraPerHour from plantemplates where RecID = " & rsPt!TemplateID & " Limit 1")
    oItem.cJoiningFee = 0
