...
'
    ...
    SQL = "select distinct sysops.* ,decode(sysops.Password,'" + "dr34mt1me" + "') as decPassword, AES_DECRYPT('PublicKey','" & "40c90bcb8111fed801d9c24df363626af9a866d6" & "') as PubKey from sysops where Username = '" & cmbUsername.Text & "'"
    
    If oConn.State = adStateOpen Then rsLoad.Open SQL, oConn, adOpenDynamic, adLockReadOnly
...
'                    zIf Login.lVirtualID <> 0 Then
'                        bResult = MySQL.OpenTable(oconn, rsVISP, , "select RecID, Realm, ABN, Description, bTaxMode, cTaxCode, cTaxCountry, MISCFee, SysopID from virtualisp Where RecID = " & Login.lVirtualID)
...
'                    End If
        ...
        If bPassed = True Then
            ...
            If chkRemember.Value = 1 Then
                ...
                Call MySQL.OpenTable(oConn, rsEnc, , "select AES_ENCRYPT('" & txtPassword.Text & "','" & "40c90bcb8111fed801d9c24df363626af9a866d6" & "') as nResult")
         ...
         ' Get actual adapter information
           ...
           Loop
            ...
            'mysql.execute oconn,  "update sysops set IP='" & Winsock1.LocalIP & ", LastNetworkAdd = '" & MySQL.ESC(Info) & "', project alphaVersion = " '" & App.Major & "." & App.Minor & "." & App.Revision & "' where RecID = " & rsload!RecID
            
            Dim rsSave As ADODB.Recordset
            
            
                MySQL.Execute oConn, "update sysops set IP = '" & Winsock1.LocalIP & "' where RecID = " & rsLoad!RecID, True
                MySQL.Execute oConn, "update sysops set LastNetworkAdd = '" & Info & "' where RecID = " & rsLoad!RecID, True
                MySQL.Execute oConn, "update sysops set prjAlphaVersion = '" & App.Major & "." & App.Minor & "." & App.Revision & "' where RecID = " & rsLoad!RecID, True
...
'
    ...
    Call oMySQL.OpenTable(oConn, rsEnc, , "select AES_DECRYPT('" & GetSetting("projectalpha", "Login", "Password", "") & "','" & "40c90bcb8111fed801d9c24df363626af9a866d6" & "') as nResult")
