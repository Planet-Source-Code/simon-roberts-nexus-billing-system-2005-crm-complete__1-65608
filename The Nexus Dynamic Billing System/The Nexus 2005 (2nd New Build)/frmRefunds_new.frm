...
Begin VB.Form frmRefunds 
   ...
   Begin VB.Image Image1 
      Height          =   870
      Left            =   60
      Picture         =   "frmRefunds_new.frx":0BC2
      Stretch         =   -1  'True
      Top             =   30
      Width           =   840
   End
...
'
    ...
    If fSearch.lRecID <> 0 Then
        ...
        bResult = MySQL.OpenTable(directConn, rsload, , "select * from invoiceout Where AccI_RecID = " & fSearch.lRecID)
...
'
    ...
    If lvTransactions.ListItems.Count > 0 Then
        ...
        'bResult = MySQL.OpenTable(directConn, rsTraxr, , "select * from refundtraxr Limit 1")
        'rsTraxr.AddNew
        ...
        Do
            Err.Clear
            rTraxrId = MySQL.GetTMPRecID("refundtraxr", directConn)
                                      
            directConn.Execute "INSERT INTO refundtraxr (RecID, RefundSerial, acci_RecID, RefundDate, Total, SysopID, VirtualID, AmountRefunded) " + _
        ...
        Case 0 'Credit Account
            'bResult = MySQL.OpenTable(directConn, rsLoad, , "select * from invoicein Limit 1")
            On Error Resume Next
            Do
                Err.Clear
                InvInID = MySQL.GetTMPRecID("invoicein", directConn)
                directConn.Execute "INSERT INTO invoicein (RecID, acci_RecID, AmountPaid, VirtualID, Checked, AmountUsed, GSTCharged, SysopID, RefundTRXID) " + _
                              "VALUES ('" & InvInID & "','" & Frame1.Tag & "','" & fAmount.cAmountRefunded & "','" & Login.lVirtualID & "','" & -1 & "','" & 0 & "','" & 0 & "','" & Login.lSysopID & "','" & rTraxrId & "')"
            Loop Until Err.Number = 0
            
            MySQL.AddReceiptItem directConn, Frame1.Tag, , InvInID, , rTraxrId, , fAmount.cAmountRefunded, "Account Credit"
        Case 1 'Fiscal Amount
            
            MySQL.AddReceiptItem directConn, Frame1.Tag, , InvInID, , rTraxrId, , fAmount.cAmountRefunded, "Fiscal Transaction"
        End Select
        ...
        For ix = 1 To lvTransactions.ListItems.Count
            Set itmX = lvTransactions.ListItems(ix)
            If itmX.Checked = True Then
                Call MySQL.OpenTable(directConn, rsload, , "select acci_RecID, TotalDue, AmountRefunded, AmountPaid from invoiceout where RecID =" & Mid(itmX.Key, 2))
                cDiff = rsload!TotalDue - rsload!AmountRefunded - rsload!AmountPaid
                If cDiff < fAmount.cAmountRefunded And cDiff <> 0 Then
                    MySQL.Execute directConn, "Update invoiceout Set AmountRefunded = AmountRefunded + " & cDiff & " where RecID = " & Mid(itmX.Key, 2)
                    MySQL.Execute directConn, "Update invoiceout Set GSTRefunded = AmountRefunded * " & oTax(Login.TaxCode, Login.TaxCountry) & " where RecID = " & Mid(itmX.Key, 2)
                    MySQL.Execute directConn, "Update invoiceout Set RefundID = " & rTraxrId & " where RecID = " & Mid(itmX.Key, 2)
                    MySQL.Execute directConn, "INSERT INTO flags_refunds (acciServiceID, Refunded, GST, SysopID, VirtualID, acci_RecID, RefundTRXID) VALUES ('" & rsload!RecID & "','" & cDiff & "','" & cDiff * oTax(Login.TaxCode, Login.TaxCountry) & "','" & Login.lSysopID & "','" & Login.lVirtualID & "','" & rsload!acci_RecID & "','" & rTraxrId & "')"
                    fAmount.cAmountRefunded = fAmount.cAmountRefunded - cDiff
                Else
                    MySQL.Execute directConn, "Update invoiceout Set AmountRefunded = AmountRefunded + " & fAmount.cAmountRefunded & " where RecID = " & Mid(itmX.Key, 2)
                    MySQL.Execute directConn, "Update invoiceout Set GSTRefunded = AmountRefunded * " & oTax(Login.TaxCode, Login.TaxCountry) & " where RecID = " & Mid(itmX.Key, 2)
                    MySQL.Execute directConn, "Update invoiceout Set RefundID = " & rTraxrId & " where RecID = " & Mid(itmX.Key, 2)
                    MySQL.Execute directConn, "INSERT INTO flags_refunds (acciServiceID, Refunded, GST, SysopID, VirtualID, acci_RecID, RefundTRXID) VALUES ('" & rsload!RecID & "','" & fAmount.cAmountRefunded & "','" & fAmount.cAmountRefunded * oTax(Login.TaxCode, Login.TaxCountry) & "','" & Login.lSysopID & "','" & Login.lVirtualID & "','" & rsload!acci_RecID & "','" & rTraxrId & "')"
                    itmX.SubItems(6) = CCur(itmX.SubItems(6)) + (fAmount.cAmountRefunded + fAmount.cAmountRefunded * oTax(Login.TaxCode, Login.TaxCountry))
                    bDone = False
                    Exit For
                End If
                'bResult = MySQL.OpenTable(directConn, rsLoad, , "select AmountRefunded, GSTRefunded from invoiceout Where RecID = " & Mid(itmX.Key, 2))
                itmX.SubItems(6) = CCur(itmX.SubItems(6)) + (fAmount.cAmountRefunded + fAmount.cAmountRefunded * oTax(Login.TaxCode, Login.TaxCountry))
                
                bDone = False
                If rsload!TraxrID <> 0 Then
                    MySQL.Execute directConn, "UPDATE invoicetraxr SET AmountCredited=AmountCredited+" & fAmount.cAmountRefunded + fAmount.cAmountRefunded * oTax(Login.TaxCode, Login.TaxCountry) & " where RecID = " & rsload!TraxrID
                    bDone = True
                End If
