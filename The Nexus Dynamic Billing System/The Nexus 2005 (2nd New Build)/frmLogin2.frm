...
'
    ...
    On Error Resume Next
    If directConn.State = adStateOpen Then rsload.Open SQL, directConn, adOpenDynamic, adLockReadOnly
    ...
    If rsload.State = adStateOpen Then
            ...
            While Not rsload.EOF And Err.Number = 0
                If rsload!DecPassword = txtPassword.Text Then
                    ...
                    INClause = IIf(IsNull(rsload!INClause), "('" & Login.lVirtualID & "')", Val(rsload!INClause))
                    
                    Call oViSP.PopulateMe(Login.lVirtualID, directConn)
                    Call oResell.PopulateResellers(directConn, fs_LoadHeader, Login.lVirtualID, "VISPDetails")
                    
                    If Not Login.lVirtualID = 1000 And Not Login.lVirtualID = 0 Then
                        bResult = MySQL.OpenTable(directConn, rsVISP, , "select RecID, Realm, ABN, Description, bTaxMode, cTaxCode, cTaxCountry, MISCFee, SysopID from virtualisp Where RecID = " & Login.lVirtualID)
        ...
        If bPassed = True Then
            ...
            If chkRemember.Value = 1 Then
                ...
                Call MySQL.OpenTable(directConn, rsEnc, , "select AES_ENCRYPT('" & txtPassword.Text & "','" & odb.colSalts.ReturnSalt("PublicKey") & "') as nResult")
         ...
         ' Get actual adapter information
           ...
           Loop
            ...
            'mysql.execute directConn,  "update sysops set IP='" & Winsock1.LocalIP & ", LastNetworkAdd = '" & MySQL.ESC(Info) & "', project alphaVersion = " '" & App.Major & "." & App.Minor & "." & App.Revision & "' where RecID = " & rsload!RecID
            
            Dim rsSave As adodb.Recordset
            
            
                MySQL.Execute directConn, "update sysops set IP = '" & Winsock1.LocalIP & "' where RecID = " & rsload!RecID, True
                MySQL.Execute directConn, "update sysops set LastNetworkAdd = '" & Info & "' where RecID = " & rsload!RecID, True
                MySQL.Execute directConn, "update sysops set prjAlphaVersion = '" & App.Major & "." & App.Minor & "." & App.Revision & "' where RecID = " & rsload!RecID, True
        ...
        Else
            ...
            If Not frmAgent.oChar Is Nothing Then frmAgent.oChar.Speak "Password or Username was incorrect!"
        End If
    Else
        If rsload.RecordCount = 0 Then
            If Not frmAgent.oChar Is Nothing Then frmAgent.oChar.Play "Decline"
            If Not frmAgent.oChar Is Nothing Then frmAgent.oChar.Speak "Username not found!"
        Else
            If Not frmAgent.oChar Is Nothing Then frmAgent.oChar.Play "Decline"
            If Not frmAgent.oChar Is Nothing Then frmAgent.oChar.Speak "Password was Incorrect!"
        End If
    End If
...
'
    ...
    Call MySQL.OpenTable(directConn, rsEnc, , "select AES_DECRYPT('" & GetSetting("projectalpha", "Login", "Password", "") & "','" & odb.colSalts.ReturnSalt("PublicKey") & "') as nResult")
...
'
    ...
    If bAgent = False Then
        ...
        If Not frmAgent.oChar Is Nothing Then
        ...
        Else
            If Not frmAgent.oChar Is Nothing Then frmAgent.oChar.MoveTo (Me.Left + Me.Width + 120) / Screen.TwipsPerPixelX, (Me.Top + Me.Height / 2) / Screen.TwipsPerPixelY
        End If
        If Not frmAgent.oChar Is Nothing Then frmAgent.oChar.Show
        If Not frmAgent.oChar Is Nothing Then frmAgent.oChar.GestureAt (Me.Left + Frame1.Left) / Screen.TwipsPerPixelX, (Me.Top + Frame1.Top) / Screen.TwipsPerPixelY
        
        If Not frmAgent.oChar Is Nothing Then frmAgent.oChar.Speak "Welcome to The Nexus, Please enter in your username and password to enter the system, this would have been provided to you by the carrier."
        bAgent = True
    End If
