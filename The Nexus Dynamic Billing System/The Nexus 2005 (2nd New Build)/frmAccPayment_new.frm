...
Begin VB.Form frmAccPayment 
   ...
   ClientTop       =   2205
   ClientWidth     =   7845
   Icon            =   "frmAccPayment_new.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   6480
...
'
    ...
    bResult = MySQL.OpenTable(directConn, rsload, , "select * from invout_payment where InvOut_RecID = " & ltmpRecID)
...
'
    ...
    For ix = 1 To lvPaymentType.ListItems.Count
        If lvPaymentType.ListItems(ix).Checked = True Then
            ...
            If Left(lvPaymentType.ListItems(ix).Key, 1) = "c" Then
                ...
                '
                '¤
                bResult = MySQL.OpenTable(directConn, rs_creditcard, , "select RecID, AccI_RecID as acci_RecID, bType, AES_DECRYPT(CardNumber,'" + odb.colSalts.ReturnSalt("CCSalt") + "" & odb.colSalts.ReturnSalt("md5Password") & "') as CardNumber, SecurityNumber, ExpiryDate, Name from creditcard where AccI_RecID = " & Me.acci_RecID)
    ...
    bResult = MySQL.OpenTable(directConn, rsload, , "select * from invoiceout Where RecID = " & Me.l_RecID & " Limit 1")
    ...
    For ix = 1 To lvPaymentType.ListItems.Count
        If lvPaymentType.ListItems(ix).Checked = True Then
            If Left(lvPaymentType.ListItems(ix).Key, 1) = "c" Then
                MySQL.AddReceiptItem directConn, rsload!acci_RecID, rsSave!RecID, , , , rsSave!TotalPaid, , "CC: " + Right(fFrmCC.screditcard, 5)
            Else
                MySQL.AddReceiptItem directConn, rsload!acci_RecID, rsSave!RecID, , , , rsSave!TotalPaid, , lvPaymentType.ListItems(ix).Text + " Payment"
            End If
        End If
    Next
    ...
    If rsload!TraxrID <> 0 Then
        ...
        bResult = MySQL.OpenTable(directConn, rsTraxr, , "select * from invoicetraxr Where RecID = " & rsload!TraxrID)
...
'
    ...
    bResult = MySQL.OpenTable(directConn, rsSave, , "select * from Flags_invoiceout Limit 0")
