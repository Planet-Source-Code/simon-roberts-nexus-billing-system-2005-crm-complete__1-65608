...
Begin VB.Form frmContracts1 
   ...
   MaxButton       =   0   'False
   MinButton       =   0   'False
   Picture         =   "frmContracts1_new.frx":0000
   ScaleHeight     =   567
   ScaleMode       =   3  'Pixel
   ScaleWidth      =   703
...
'
    ...
    If Not frmAgent.oChar Is Nothing Then frmAgent.oChar.Speak "Creating Contract and Setup Fee Matrix"
    If Not frmAgent.oChar Is Nothing Then frmAgent.oChar.Play "Processing"
    
    
    Dim rsload As adodb.Recordset
    
    Call MySQL.OpenTable(directConn, rsload, , "select TemplateID from plantypes where RecID = " & ptRecID & "")

    
    Dim itmy As ListItem
    
    Call MySQL.Execute(directConn, "delete from contractsruntime where ptRecID = '" & ptRecID & "'")
    
    For li = 1 To pb.Max - 1
        Set itmy = lvContracts.ListItems(li)
        If itmy.Checked = True Then
                            
            Call MySQL.Execute(directConn, "INSERT INTO contractsruntime (ptRecID, TemplateID, ContractID, VirtualID, Termination, JoiningFee, PeriodFee, FeePerBlock, FeePerHour) " + _
...
'
    ...
    Call GUI.LoadColWidths(lvContracts, Me)
        ...
        Call MySQL.OpenTable(directConn, rsload, , "select * from contracttemplates where ptRecID = " & rsload!TemplateID & " and bDeleted = 0")
