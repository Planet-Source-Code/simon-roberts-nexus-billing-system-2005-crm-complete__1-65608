...
Begin VB.Form frmImpContracts 
   ...
   ClientTop       =   4620
   ClientWidth     =   14520
   Icon            =   "frmImpContracts_new.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   509
...
'
    ...
    If Not frmAgent.oChar Is Nothing Then frmAgent.oChar.Play "Writing"
    ...
    For il = 1 To pb.Max - 1
      ...
      Do
          Err.Clear
          lvContracts.Tag = MySQL.GetTMPRecID("contracttemplates", directConn)
          Call MySQL.Execute(directConn, "INSERT into contracttemplates (RecID, ptRecID) VALUES ('" & lvContracts.Tag & "','" & Me.ptRecID & "')")
      Loop Until Err.Number = 0
      

    Call MySQL.OpenTable(directConn, rsload, , "select PeriodFee from plantemplates where RecID = '" & ptRecID & "'")
        ...
        Call MySQL.OpenTable(directConn, rsScan, , "select * from flags_tempextras where ContractID = '" & Mid(itmy.Key, 2) & "'")
        ...
        If rsScan.State = adStateOpen Then
            If rsScan.RecordCount > 0 Then
                While Not rsScan.EOF And Err.Number = 0
                    ...
                    Call MySQL.Execute(directConn, "INSERT INTO flags_tempextras (PlanType, NumberOf, Checked, ContractID) VALUES ('" & Val(rsScan!PlanType) & "','" & Val(rsScan!NumberOf) & "','" & Val(rsScan!Checked) & "','" & Val(lvContracts.Tag) & "')")
...
'
    ...
    bResult = MySQL.OpenTable(directConn, rsload, "servicetypes")
...
'
    ...
    End If
        ...
        Call MySQL.OpenTable(directConn, rsload, , "select * from contracttemplates where ptRecID = " & Mid(Item.Key, 2) & " and bDeleted = 0")
        Dim itmX As ListItem
        Dim bx As Byte
...
'    Frame1(0).Caption = "Services and Plans: " & Node.Text
 ...
 '   Command1.Enabled = False
    ...
    bResult = MySQL.OpenTable(directConn, rsPlanTemp, , "select * from plantemplates Where ServiceID = " & Mid(Node.Key, 2))
