...
Begin VB.Form frmAccountTypes 
   ...
   EndProperty
   Icon            =   "frmAccountTypes_New.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   11820
   ...
   Begin VB.Image Image1 
      Height          =   900
      Left            =   -30
      Picture         =   "frmAccountTypes_New.frx":0442
      Stretch         =   -1  'True
      Top             =   -60
      Width           =   930
   End
...
'
    ...
    If Not tvServiceTypes.SelectedItem Is Nothing Then
        ...
        If ffrmTemp.vRecID <> 0 Then
            ...
            bResult = MySQL.OpenTable(directConn, rsload, , "select * from plantemplates Where RecID = " & ffrmTemp.vRecID)
            ...
            bResult = MySQL.OpenTable(directConn, rsload, , "select plantypes.Description, flags_tempextras.* from plantypes, flags_tempextras Where flags_tempextras.PlanType = plantypes.RecID AND flags_tempextras.ptRecID = " & rsload!RecID)
...
'
    ...
    bResult = MySQL.OpenTable(directConn, rsPSType, , "select * from plantypes Where RecID = " & Val(RecID))
    bResult = MySQL.OpenTable(directConn, rsServices, , "select BillImmediately from servicetypes where RecID = " & Mid(tvServiceTypes.SelectedItem.Key, 2))
    ...
    Case "FTP", "POP3", "WWW", "DESIGN", "DOMAIN", "GATEWAY", "TRAINING", "SALES", "CONSULT", "COLO", "HOSTING", "ALIAS", "HOST"
        ...
        Case 0
            ...
            Do
                On Error Resume Next
                Err.Clear
                RecID = MySQL.GetTMPRecID("plantypes", directConn)
                Call MySQL.Execute(directConn, "Insert into plantypes (RecID, Description, PeriodFee, JoiningFee, Rollover, ServiceID, VirtualID, BillImmediately, TemplateID, MbQuota, VendorID ) " + "VALUES('" & RecID & "','" & MySQL.ESC(IIf(txtDesc <> "", txtDesc, txtDescription)) & "','" & Val(txtFee(3)) & "','" & Val(txtFee(5)) & "','" & chkLimit(2).Value & "','" & Val(Mid(tvServiceTypes.SelectedItem.Key, 2)) & "','" & txtFee(4).Tag & "','" & rsServices!BillImmediately & "','" & txtFee(0).Tag & "','" & lblMBQuota(0).Tag & "','" & VendorID & "')")
            Loop Until Err.Number = 0
            
            For bx = 0 To 2
                If optBillingCycle(bx).Value = True Then
                    Call MySQL.Execute(directConn, "update plantypes set roIntervalType = '" & optBillingCycle(bx).Tag & "' where RecID = " & RecID)
                    Call MySQL.Execute(directConn, "update plantypes set roInterval = '" & Val(IIf(txtBillingCycle(bx).Text = "", "1", txtBillingCycle(bx).Text)) & "' where RecID = " & RecID)
                    If cmbRollover.ListIndex <> -1 Then
                        Call MySQL.Execute(directConn, "update plantypes set roRecID = '" & cmbRollover.ItemData(cmbRollover.ListIndex) & "' where RecID = " & RecID)
                    Else
                        Call MySQL.Execute(directConn, "update plantypes set roRecID = '" & 0 & "' where RecID = " & RecID)
                    End If
                    Exit For
                End If
            Next
            ...
            For bx = 3 To 5
                If optBillingCycle(bx).Value = True Then
                    Call MySQL.Execute(directConn, "update plantypes set chgIntervalType = '" & optBillingCycle(bx).Tag & "' where RecID = " & RecID)
                    Call MySQL.Execute(directConn, "update plantypes set chgInterval = '" & txtBillingCycle(bx).Text & "' where RecID = " & RecID)
                    Exit For
                End If
            Next

            Call MySQL.Execute(directConn, "update plantypes set MBPerPeriod = -1 where RecID = " & RecID)
            Call MySQL.Execute(directConn, "update plantypes set MBBlockSize = 0 where RecID = " & RecID)
            Call MySQL.Execute(directConn, "update plantypes set FeePerBlock = 0 where RecID = " & RecID)
            Call MySQL.Execute(directConn, "update plantypes set HoursPerPeriod = -1 where RecID = " & RecID)
            Call MySQL.Execute(directConn, "update plantypes set ExtraPerHour = 0 where RecID = " & RecID)
            Call MySQL.Execute(directConn, "update plantypes set BillOnce = '" & chkBillOnce.Value & "' where RecID = " & RecID)
            Call MySQL.Execute(directConn, "update plantypes set CatNo = '" & txtCat(0) & "' where RecID = " & RecID)
            Call MySQL.Execute(directConn, "update plantypes set CostPrice = '" & txtFee(3).Tag & "' where RecID = " & RecID)
            
        Case Else
            
                        
            If MySQL.OpenTable(directConn, rsSave, , "select * from plantypes where RecID = " & RecID) = True Then
                ...
                If rsSave.RecordCount > 0 Then
                    ...
                    If rsSave!Description <> txtDescription Then
                        ...
                        MySQL.Execute directConn, "Update accountviewer Set Description='View " & MySQL.ESC(txtDescription) & " Users' Where selectStatement Like '%" & rsSave!RecID & "%' and Description <> 'Active' and Description <> 'Cancelled'"
                        
                    End If
                    
                    Call MySQL.Execute(directConn, "update plantypes set Description ='" & MySQL.ESC(IIf(txtDesc <> "", txtDesc, txtDescription)) & "' where RecID = " & RecID)
                    Call MySQL.Execute(directConn, "update plantypes set PeriodFee = '" & Val(txtFee(3)) & "' where RecID = " & RecID)
                    Call MySQL.Execute(directConn, "update plantypes set JoiningFee = '" & Val(txtFee(5)) & "' where RecID = " & RecID)
                    Call MySQL.Execute(directConn, "update plantypes set Rollover = '" & chkLimit(2).Value & "' where RecID = " & RecID)
                    Call MySQL.Execute(directConn, "update plantypes set BillImmediately = '" & rsServices!BillImmediately & "' where RecID = " & RecID)
                    Call MySQL.Execute(directConn, "update plantypes set TemplateID = '" & txtFee(0).Tag & "' where RecID = " & RecID)
                    Call MySQL.Execute(directConn, "update plantypes set MbQuota = '" & lblMBQuota(0).Tag & "' where RecID = " & RecID)
                    Call MySQL.Execute(directConn, "update plantypes set TemplateID = '" & txtFee(0).Tag & "' where RecID = " & RecID)
                    Call MySQL.Execute(directConn, "update plantypes set VendorID = '" & VendorID & "' where RecID = " & RecID)
                    Call MySQL.Execute(directConn, "update plantypes set CatNo = '" & txtCat(0) & "' where RecID = " & RecID)
                    Call MySQL.Execute(directConn, "update plantypes set MbQuota = '" & lblMBQuota(0).Tag & "' where RecID = " & RecID)
                    Call MySQL.Execute(directConn, "update plantypes set TemplateID = '" & txtFee(0).Tag & "' where RecID = " & RecID)
                    Call MySQL.Execute(directConn, "update plantypes set CostPrice = '" & txtFee(3).Tag & "' where RecID = " & RecID)
                    ...
                    For bx = 3 To 5
                        If optBillingCycle(bx).Value = True Then
                            MySQL.Execute directConn, "Update plantypes Set chgIntervalType = '" & optBillingCycle(bx).Tag & "', chgInterval = " & txtBillingCycle(bx).Text & " where RecID = " & rsSave!RecID
                            Exit For
                        End If
                    Next
    ...
    Case "DIALUP", "ADSL", "SHDSL"
        ...
        Case 0
            ...
            If chkLimit(1).Value = 1 Then
                Call MySQL.Execute(directConn, "update plantypes set MBPerPeriod = '" & Val(txtMB(1)) & "' where RecID = " & RecID)
                Call MySQL.Execute(directConn, "update plantypes set MBBlockSize = '" & Val(txtMB(0)) & "' where RecID = " & RecID)
                Call MySQL.Execute(directConn, "update plantypes set FeePerBlock = '" & Val(txtFee(1)) & "' where RecID = " & RecID)
            Else
                Call MySQL.Execute(directConn, "update plantypes set MBPerPeriod = '" & 1 & "' where RecID = " & RecID)
                Call MySQL.Execute(directConn, "update plantypes set MBBlockSize = '" & 0 & "' where RecID = " & RecID)
                Call MySQL.Execute(directConn, "update plantypes set FeePerBlock = '" & 0 & "' where RecID = " & RecID)
            ...
            If chkLimit(0).Value = 1 Then
                Call MySQL.Execute(directConn, "update plantypes set HoursPerPeriod = '" & txtHours & "' where RecID = " & RecID)
                Call MySQL.Execute(directConn, "update plantypes set ExtraPerHour = '" & Val(txtFee(2)) & "' where RecID = " & RecID)
            Else
                Call MySQL.Execute(directConn, "update plantypes set HoursPerPeriod = '" & -1 & "' where RecID = " & RecID)
                Call MySQL.Execute(directConn, "update plantypes set ExtraPerHour = '" & 0 & "' where RecID = " & RecID)
            End If
        ...
        Case Else
            ...
            If MySQL.OpenTable(directConn, rsSave, , "select * from plantypes where RecID = " & RecID) = True Then
                If rsSave.RecordCount > 0 Then
                    If rsSave!Description <> txtDescription Then
                        MySQL.Execute directConn, "Update accountviewer Set Description='View " & MySQL.ESC(txtDesc) & " Users' Where selectStatement Like '\'" & rsSave!RecID & "\'' and Description <> 'Active' and Description <> 'Cancelled'"
                        'rsSave!Description = txtDescription
                    End If
        
                    If rsSave.RecordCount > 0 Then
                        Call MySQL.Execute(directConn, "update plantypes set Description = '" & MySQL.ESC(txtDesc) & "' where RecID = " & RecID)
                        Call MySQL.Execute(directConn, "update plantypes set PeriodFee = '" & Val(txtFee(0)) & "' where RecID = " & RecID)
                        Call MySQL.Execute(directConn, "update plantypes set JoiningFee = '" & Val(txtFee(4)) & "' where RecID = " & RecID)
                        Call MySQL.Execute(directConn, "update plantypes set Rollover = '" & chkLimit(2).Value & "' where RecID = " & RecID)
                        Call MySQL.Execute(directConn, "update plantypes set BillImmediately = '" & rsServices!BillImmediately & "' where RecID = " & RecID)
                        Call MySQL.Execute(directConn, "update plantypes set RadiusID = '" & cmdRadius.ItemData(cmdRadius.ListIndex) & "' where RecID = " & RecID)
                        Call MySQL.Execute(directConn, "update plantypes set TemplateID = '" & txtFee(0).Tag & "' where RecID = " & RecID)
                        Call MySQL.Execute(directConn, "update plantypes set VirtualID = '" & txtFee(4).Tag & "' where RecID = " & RecID)
                        Call MySQL.Execute(directConn, "update plantypes set VendorID = '" & VendorID & "' where RecID = " & RecID)
                        Call MySQL.Execute(directConn, "update plantypes set CatNo = '" & txtCat(0) & "' where RecID = " & RecID)
                        Call MySQL.Execute(directConn, "update plantypes set CostPrice = '" & txtFee(3).Tag & "' where RecID = " & RecID)
    ...
    MySQL.Execute directConn, "Update plantypes set Description = '" & MySQL.ESC(descrip) & "' where RecID = " & RecID
    MySQL.Execute directConn, "Update plantypes set CatNo = '" & MySQL.ESC(txtCat(0)) & "' where RecID = " & RecID
    
    Dim ox As Byte
    For ox = 0 To 3
        If Option1(ox).Value = 1 Then
            MySQL.Execute directConn, "Update plantypes set OptionalText = '" & MySQL.ESC(Option1(ox).Caption) & "' where RecID = " & RecID
        End If
    Next
    ...
    Call MySQL.OpenTable(directConn, rsSave, , "select * from plantypes where RecID = '" & RecID & "'")
    
    If bNEW = False Then
        Call MySQL.fillLV(directConn, rsSave, lvAccounts, True, lvAccounts.SelectedItem)
    Else
        Call MySQL.fillLV(directConn, rsSave, lvAccounts, True)
    End If
...
'
    ...
    'MySQL.Execute directConn, "Update accountviewer Set Description='View " & MySQL.ESC(NewString) & " Users' Where selectStatement Like '\'" & txtDescription.Tag & "\'' and Description <> 'Active' and Description <> 'Cancelled'"
    MySQL.Execute directConn, "Update plantypes Set `" & Left(lvAccounts.ColumnHeaders(1).Tag, InStr(lvAccounts.ColumnHeaders(1).Tag, ">") - 1) & "`='" & MySQL.ESC(NewString) & "' Where RecID = " & txtDescription.Tag
...
'
    ...
    Dim rsPSType As adodb.Recordset
    bResult = MySQL.OpenTable(directConn, rsPSType, , "select * from plantypes Where RecID = " & Mid(Item.Key, 2))
    ...
    If rsPSType.State = adStateOpen Then
        If rsPSType.RecordCount > 0 Then
            ...
            For ox = 0 To 3
                If Option1(ox).Value = 1 Then
                    MySQL.Execute directConn, "Update plantypes set OptionalText = '" & MySQL.ESC(Option1(ox).Caption) & "' where RecID = " & ltmpRecID
                End If
            Next
...
'
    ...
    Case vbKeyDelete
        ...
        Case vbYes
            MySQL.Execute directConn, "Delete from plantypes where RecID = " & Mid(lvAccounts.SelectedItem.Key, 2)
            MySQL.Execute directConn, "Delete from accountviewer where selectStatement Like '%\'" & Mid(lvAccounts.SelectedItem.Key, 2) & "\'%'"
    ...
    Case vbKeyF12
            ...
            Do
                Err.Clear
                ltmpRecID3 = MySQL.GetTMPRecID("accountviewer", directConn)
...
'
    ...
    bResult = MySQL.OpenTable(directConn, rsPSType, , MySQL.virtualisp("select * from plantypes Where plantypes.ServiceID = '" & Mid(Node.Key, 2) + "'", "plantypes", True))
    
    Call MySQL.fillLV(directConn, rsPSType, lvAccounts, False)
    ...
    If lvPlans.ListItems.Count > 0 Then
        ...
        For sa = 1 To lvPlans.ListItems.Count
            ...
            If Left(itmX.Key, 1) = "r" Then
                ...
                Call MySQL.OpenTable(directConn, rsSave, , "select * from flags_plantype Limit 0")
            ...
            ElseIf Left(itmX.Key, 1) = "e" Then
                ...
                Call MySQL.OpenTable(directConn, rsSave, , "select * from flags_plantype where RecID = " & Mid(itmX.Key, 2) & " Limit 1")
