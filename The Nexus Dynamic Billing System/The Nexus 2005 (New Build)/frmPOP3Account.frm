...
Begin VB.Form frmPOP3Account 
   ...
   Begin VB.Image Image1 
      Height          =   1410
      Left            =   0
      Picture         =   "frmPOP3Account.frx":0442
      Stretch         =   -1  'True
      Top             =   0
      Width           =   1215
   End
...
'
    ...
    If MySQL.OpenTable(ADOConn, rsCheck, , "select * from acci_services Where ServiceID = 4 and Username = '" & txtField(1) & "' and BaseURL = '" & cmbDomain.Text & "' Limit 1") = True Then
...
'
    ...
    If txtField(1).Tag <> txtField(1).Text Then
        If MySQL.OpenTable(ADOConn, rsCheck, , "select * from acci_services Where ServiceID = 4 and Username = '" & txtField(1).Text & "' and BaseURL = '" & cmbDomain.Text & "' Limit 1") = True Then
            If rsCheck.RecordCount > 0 Then
    ...
    Case vbYes
        ...
        ' CHANGES
        'MySQL.Execute ADOConn, "Update acci_services Set ContactName='" & MySQL.ESC(txtField(0)) & "', BaseURL = '" & cmbDomain.Text & "', Username='" & MySQL.ESC(txtField(1)) & "', Password=AES_ENCRYPT('" & txtField(2) & "','" & odb.colSalts.ReturnSalt("md5Password") & "'), DynamicField1='" & IIf(chkSpam.Value = 1, "Scanning", "No Scan") & "' where RecID = " & Mid(lvEmail.SelectedItem.Key, 2)
        'MySQL.Execute ADOConn, "Update acci_aliases set dest='" & MySQL.ESC(txtField(1) + "@" + cmbDomain.Text) & "' where dest = '" & MySQL.ESC(lvEmail.SelectedItem.SubItems(1) + "@" + lvEmail.SelectedItem.SubItems(3)) & "'"
...
'
    ...
    If MySQL.OpenTable(ADOConn, rsLoad, , "select count(*) as recordcount, Sum(DynamicField2) as NumAllowed from acci_services where ptRecID = " & Me.ptRecID & " and AccI_RecID = " & oCNT.fRecID) = True Then
