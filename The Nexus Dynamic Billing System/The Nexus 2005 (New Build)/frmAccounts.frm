...
'
    ...
    bResult = MySQL.OpenTable(ADOConn, rsload, , MySQL.virtualisp("select Count(distinct invoiceout.RecID) as RecordCount from invoiceout, accountinfo Where accountinfo.RecID = invoiceout.AccI_RecID AND (invoiceout.AmountRefunded + invoiceout.GSTRefunded + invoiceout.AmountPaid) < invoiceout.TotalDue group by virtualisp.VirtualID Order By accountinfo.AccountName", "invoiceout") + " Limit 1")
        ...
        For ix = 0 To iRecCount Step 30
            ...
            Case Else
                ...
                While Not rsload.EOF And Err.Number = 0
                    ...
                    pb1.Refresh
                    'gSleep
                    Set itmX = lvReceivables.ListItems.Add(, "r" & rsload!RecID, IIf(IsNull(rsload!AccountName), "(null)", rsload!AccountName))
    ...
    bResult = MySQL.OpenTable(ADOConn, rsload, , MySQL.virtualisp("select Count(distinct invoicein.RecID) as RecordCount from invoicein, accountinfo Where accountinfo.RecID = invoicein.AccI_RecID AND AmountPaid > AmountUsed group by virtualisp.VirtualID", "invoicein") + " Limit 1")
    ...
    If iRecCount <> 0 Then
        ...
        lvExtra.ListItems.Clear
        For ix = 0 To iRecCount Step 30
            
            bResult = MySQL.OpenTable(ADOConn, rsload, , MySQL.virtualisp("select distinct accountinfo.AccountName ,invoicein.AmountPaid ,invoicein.PaidWhen ,invoicein.AmountUsed ,invoicein.Sub ,invoicein.GSTCharged ,invoicein.RecID ,invoicein.TotalPaid from invoicein, accountinfo Where accountinfo.RecID = invoicein.AccI_RecID AND AmountPaid > AmountUsed", "invoicein") + " Limit " & ix & ",30")
            ...
            Case Else
                ...
                While Not rsload.EOF And Err.Number = 0
                    ...
                    Set itmX = lvExtra.ListItems.Add(, "r" & rsload!RecID, rsload!AccountName)
