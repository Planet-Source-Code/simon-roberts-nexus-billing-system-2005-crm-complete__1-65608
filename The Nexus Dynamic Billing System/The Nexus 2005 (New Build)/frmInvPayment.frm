...
'
    ...
    For ix = 1 To lvPaymentType.ListItems.Count
        If lvPaymentType.ListItems(ix).Checked = True Then
            ...
            If Left(lvPaymentType.ListItems(ix).Key, 1) = "c" Then
                ...
                '
                '¤
                Call MySQL.OpenTable(ADOConn, rs_creditcard, , "select * from creditcard where AccI_RecID = " & Me.acci_RecID)
    ...
    bResult = MySQL.OpenTable(ADOConn, rsLoad, , "select * from invoicetraxr Where RecID = " & Me.l_RecID & " Limit 1")
    
    
    If chkGST.Value = 1 Then
        
        Call MySQL.Execute(ADOConn, "Insert into invtrx_payment (InvTrxRecID, Amount, Sub, GST, acci_RecID, WhenPaid, SysopID, VirtualID) " + _
    ...
    MySQL.Execute ADOConn, "UPDATE invoicetraxr set AmountPaid = AmountPaid + " & c_TotalPaid & " where RecID = '" & Me.l_RecID & "'"
    MySQL.Execute ADOConn, "UPDATE invoicetraxr set PaidWhen = '" & Format(sysNOW, "yyyy-mm-dd ttttt") & "' where RecID = '" & Me.l_RecID & "'"
    'End If
    
    
    
    If rsLoad!AmountPaid + c_TotalPaid >= rsLoad!TotalDue Then
        MySQL.Execute ADOConn, "UPDATE invoicetraxr set Finalised = '-1' where RecID = '" & Me.l_RecID & "'"
    End If
...
'
    ...
    For ix = 1 To lvPaymentType.ListItems.Count
        If lvPaymentType.ListItems(ix).Checked = True Then
            ...
            MySQL.Execute ADOConn, "INSERT INTO flags_invoicetrx (InvTrx_RecID, FlagID, SysopID, VirtualID) VALUES ('" & lRecID & "','" & Mid(lvPaymentType.ListItems(ix).Key, 2) & "','" & Login.lSysopID & "','" & Login.lVirtualID & "')"
...
'
    ...
    bResult = MySQL.OpenTable(ADOConn, rsLoad, , "select * from invoiceout where TraxrID = " & vRecID)
    ...
    Do While Not rsLoad.EOF And Err.Number = 0
        If rsLoad!AmountPaid < rsLoad!TotalDue Then
            ...
            For ix = 1 To lvPaymentType.ListItems.Count
                If lvPaymentType.ListItems(ix).Checked = True Then
                    If Left(lvPaymentType.ListItems(ix).Key, 1) = "c" Then
                        MySQL.AddReceiptItem ADOConn, rsLoad!acci_RecID, rsLoad!RecID, , , , cDiff, , lvPaymentType.ListItems(ix).Text + " Payment", , Me.l_RecID
                    Else
                        MySQL.AddReceiptItem ADOConn, rsLoad!acci_RecID, rsLoad!RecID, , , , cDiff, , lvPaymentType.ListItems(ix).Text + " Payment", , Me.l_RecID
                    End If
                End If
